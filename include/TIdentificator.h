...
public:
  ...
  /***           Inline functions          ***/
  /* (located at include/TIdentificator.icc) */
  
  // HEADER bank
  Float_t NEvent();                             // inline
  
  // EVNT bank
  // Double_t Betta(Int_t k);                   // inline, was duplicated
  Double_t Betta(Int_t k, Bool_t kind = 0);     // inline
  ...
  Bool_t SampFracCheck(Int_t ke, TString tt = ""); // osoto_mod, borquez_mod
  ...
  Double_t ThetaVirtLab(Int_t ke, Bool_t kind = 0);
  Double_t PhiVirtLab(Int_t ke, Bool_t kind = 0);
  Double_t ThetaPQ(Int_t k, Int_t ke, Bool_t kind = 0);
  Double_t PhiPQ(Int_t k, Int_t ke, Bool_t kind = 0);
  Double_t CosThetaPQ(Int_t k, Int_t ke, Bool_t kind = 0);
  // Double_t PTrans2PQ(Int_t k, Bool_t kind = 0); // deprecated, borquez_mod
  // Double_t PLong2PQ(Int_t k, Bool_t kind = 0); // deprecated, borquez_mod
  Int_t Sector(Int_t k, Bool_t kind = 0);

  // Added in hayk's code
  Double_t Pt2(Int_t k, Int_t ke, Bool_t kind = 0);
  Double_t Pl2(Int_t k, Int_t ke, Bool_t kind = 0);
  // Double_t PlCM(Int_t, Bool_t = 0); // deprecated, borquez_mod
  // Double_t PmaxCM(Bool_t = 0); // deprecated, borquez_mod

  // Electron variables
  Double_t Q2(Int_t ke, Bool_t kind = 0); // ke must be electron, wherever it is, borquez_mod
  Double_t W(Int_t ke, Bool_t kind = 0);  // borquez_mod
  Double_t Nu(Int_t ke, Bool_t kind = 0); // borquez_mod
  Double_t Xb(Int_t ke, Bool_t kind = 0); // borquez_mod
  Double_t Yb(Int_t ke, Bool_t kind = 0); // borquez_mod
  
  // Hadron variables
  Double_t Zh(Int_t k, Int_t ke, Bool_t kind, Double_t Mass);
  // Double_t Xf(Int_t, Bool_t = 0);  // deprecated, borquez_mod
  Double_t Mx2(Int_t k, Int_t ke, Bool_t kind, Double_t Mass);
  Double_t T(Int_t k, Int_t ke, Bool_t kind, Double_t Mass);

  // Correction functions
  Double_t TimeCorr4(Int_t k, Int_t ke, Double_t mass);
  TVector3 *GetCorrectedVert(Int_t ke);

  //Added in hayk's code
  //Int_t ElecVertTarg(Bool_t = 0);

  // Particle Identification cuts (at src/Categorization.C)
  TString GetCategorization(Int_t k, Int_t ke, TString tt = "");
  // TString GetCategorizationMin(Int_t k); // deprecated
  ...
  Double_t FidThetaMin(Int_t ke);
  Double_t FidFunc(Int_t ke, Int_t side, Int_t param);
  Double_t FidPhi(Int_t k, Bool_t kind = 0);
  Double_t FidPhiMin(Int_t ke);
  Double_t FidPhiMax(Int_t ke);
  Bool_t FidCheckCut(Int_t ke); // ke must be electron, borquez_mod
  // Int_t FidSector(Int_t k, Bool_t kind = 0); // deprecated

  // Target methods
  Int_t ElecVertTarg(Int_t ke, Bool_t kind); // borquez_mod
  // Bool_t PionVertTarg(Int_t k); // deprecated, borquez_mod
