TString TIdentificator::GetCategorization(Int_t k, Int_t ke, TString tt) {
  ...
  if (number_dc != 0) {
    ...
    Bool_t ID_pip =
	      (Charge(k) == 1) &&
	      (((P >= 2.7) && (Nphe(k) > 25)) ||
	       ((P > 2.7 && P <= 3.3 && T4+0.08 >= -0.5 && T4+0.08 <= 0.4 && Mass2(k) < 0.4) ||     // added from SM analysis
		(P > 3.3 && P <= 3.7 && T4+0.08 >= -0.32 && T4+0.08 <= 0.32 && Mass2(k) < 0.37) ||  // added from SM analysis
		(P > 3.7 && T4+0.08 >= -0.3 && T4+0.08 <= 0.3 && Mass2(k) < 0.37)) ||               // added from SM analysis
